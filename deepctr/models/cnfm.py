...
def CNFM(linear_feature_columns, dnn_feature_columns, dnn_hidden_units=(128, 128),
    ...
    dnn_output_disentangle = DNN(dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout, False, seed=seed)(dnn_input)
    dnn_predict_disentangle = tf.keras.layers.Dense(
        1, use_bias=False, kernel_initializer=tf.keras.initializers.glorot_normal(seed))(dnn_output_disentangle)
    output_disentangle = PredictionLayer(task, name='disentangle')(dnn_predict_disentangle)

    model = tf.keras.models.Model(inputs=inputs_list, outputs=[output, output_disentangle])
    return model
