...
class FMVec(Layer):
    ...
    def __init__(self, **kwargs):
        ...
        super(FMVec, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        super(FMVec, self).build(input_shape)  # Be sure to call this somewhere!
...
class ProbAttention(Layer):
    ...
    def _update_context(self, context_in, V, scores, index, L_Q):
        ...
        batch_indexes = tf.tile(tf.range(B)[:, tf.newaxis, tf.newaxis], (1, tf.shape(V)[1], tf.shape(index)[-1]))
        head_indexes = tf.tile(tf.range(tf.shape(V)[1])[tf.newaxis, :, tf.newaxis], (B, 1, tf.shape(index)[-1]))
...
class AttentionLayer(Layer):
    def __init__(self, attention, d_model, n_heads, d_keys=None, d_values=None, normalize_query=True):
    ...
    def build(self, inputs_shape):
        ...
        if self.normalize_query:
          self.beta= tf.Variable(tf.zeros(params_shape))
          self.gamma = tf.Variable(tf.ones(params_shape))
...
class DestineLayer(Layer):
    ...
    def __init__(self, att_embedding_size=8, head_num=2, use_res=True, seed=1024, **kwargs):
        ...
        self.use_res = use_res
        self.seed = seed
        super(DestineLayer, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.W_Value = self.add_weight(name='value', shape=[embedding_size, self.att_embedding_size * self.head_num],
                                       dtype=tf.float32,
                                       initializer=tf.keras.initializers.TruncatedNormal(seed=self.seed + 2))
        self.W_Weight = self.add_weight(name='weight', shape=[embedding_size, self.att_embedding_size * self.head_num],
                                       dtype=tf.float32,
        ...
        # Be sure to call this somewhere!
        super(DestineLayer, self).build(input_shape)
    ...
    def call(self, inputs, **kwargs):
        ...
        querys = tf.tensordot(inputs, self.W_Query,
                              axes=(-1, 0))  # None F D*head_num
        querys_prime = tf.tensordot(inputs, self.W_Weight,
                              axes=(-1, 0))  # None F D*head_num
        keys = tf.tensordot(inputs, self.W_key, axes=(-1, 0))
        values = tf.tensordot(inputs, self.W_Value, axes=(-1, 0))
        ...
        # head_num None F D
        querys = tf.stack(tf.split(querys, self.head_num, axis=2))
        querys_prime = tf.stack(tf.split(querys_prime, self.head_num, axis=2))
        keys = tf.stack(tf.split(keys, self.head_num, axis=2))
        values = tf.stack(tf.split(values, self.head_num, axis=2))

        # head_num None 1 D
        mu_querys = tf.reduce_mean(querys, axis=2, keepdims=True)
        mu_querys_prime = tf.reduce_mean(querys_prime, axis=2, keepdims=True)
        mu_keys = tf.reduce_mean(keys, axis=2, keepdims=True)

        querys = querys - mu_querys
        keys = keys - mu_keys

        pairwise = tf.matmul(querys, keys, transpose_b=True) # head_num None F F
        pairwise = softmax(pairwise)

        unary = tf.matmul(keys + mu_keys, mu_querys_prime, transpose_b=True) # head_num None F 1
        unary = softmax(unary, dim=2)
        unary = tf.transpose(unary, [0, 1, 3, 2])

        self.normalized_att_scores = pairwise + unary
